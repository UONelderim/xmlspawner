...
namespace Server.Engines.XmlSpawner2
{
	public class XmlFactionMastery : XmlAttachment
	{
		...
		{
			get => m_PercentCap;
			set => m_PercentCap = value;
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public double FactionScaling
		{
			get => m_FactionScaling;
			set => m_FactionScaling = value;
		}
		...
		public override void OnWeaponHit(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven)
		{
			...
			if (defender != null && attacker != null && m_EnemyType != XmlMobFactions.GroupTypes.End_Unused)
			{
				...
				if (list != null && list.Count > 0)
				{
					...
					var glist = XmlMobFactions.FindGroups(defender);

					if (glist != null && glist.Count > 0)
						foreach (var targetgroup in glist)
							...
							if (targetgroup == m_EnemyType)
							{
								...
								var g = XmlMobFactions.FindGroup(m_EnemyType);
		...
		public override string OnIdentify(Mobile from)
		{
			...
			if (Expiration > TimeSpan.Zero)
				...
				msg = String.Format("Faction Mastery : +{2}% damage vs {0} ({3}% max), {1}% hitchance", m_Enemy, Chance,
					percentincrease, PercentCap);
		...
		private int ComputeIncrease(int faclevel)
		{
			...
			var val = (int)(PercentIncrease + faclevel * FactionScaling);
