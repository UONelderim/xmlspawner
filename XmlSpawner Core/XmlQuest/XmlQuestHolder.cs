...
namespace Server.Items
{
	public abstract class XmlQuestHolder : Container, IXmlQuest
	{
		...
		{
			if (item == this) return base.CheckLift(@from, item, ref reject);
		...
		private void UnHideRewards()
		{
			...
			if (Items == EmptyItems)
			{
				...
				if (tmpitem == null || tmpitem.Deleted) tmpitem = new Item(1);
			...
			if (tmpitem != null && tmpitem != PlaceHolderItem) tmpitem.Delete();
		}
		...
		{
			list.Add(Name);
			if (LootType == LootType.Blessed) list.Add(1038021);
			...
			if (RootParent is PlayerVendor) ((PlayerVendor)RootParent).GetChildProperties(list, this);
		}
		...
		private void VerboseAddJournalEntry(string entrystring, bool notify, bool echo)
		{
			...
			// parse the value
			var args = entrystring.Split(colondelim, 2);
		...
		private void CalculateWeight(Item target)
		{
			if (target is Container)
			{
				...
				foreach (var i in ((Container)target).Items)
				{
		...
		private void UpdateWeight()
		{
			...
			if (RootParent is Mobile) ((Mobile)RootParent).UpdateTotals();
		...
		public Item RewardItem
		{
			get
			{
				...
				{
					var o = XmlQuest.CreateItem(this, m_RewardString, out m_status_str, typeof(Item));
					if (o is Item)
					{
		...
		public void CheckAutoReward()
		{
			if (!Deleted && AutoReward && IsCompleted && Owner != null &&
			    (RewardItem != null && !m_RewardItem.Deleted ||
			     RewardAttachment != null && !m_RewardAttachment.Deleted || RewardAction != null))
			{
				if (RewardItem != null)
				{
				...
				{
					Timer.DelayCall(new TimerStateCallback(AttachToCallback),
						new object[] { Owner, m_RewardAttachment });

					m_RewardAttachment = null;
				}

				if (RewardAction != null) BaseXmlSpawner.ExecuteActions(Owner, Owner, RewardAction);
