	...
	public class XmlFindGump : Gump
	{
		public class XmlFindThread
		{
			...
			public void XmlFindThreadMain()
			{
				...
				var results = Search(m_SearchCriteria, out status_str);
			...
			public void GumpDisplayCallback(object state)
			{
				...
				{
					from.SendGump(gump);
					if (status_str != null) @from.SendMessage(33, "XmlFind: {0}", status_str);
				}
			}
		}
		...
		public class SearchCriteria
		{
			...
			public SearchCriteria(bool dotype, bool doname, bool dorange, bool doregion, bool doentry, bool doentrytype,
				...
				bool domal, bool doilsh, bool dotok, bool doint, bool donull, bool doerr, bool doage, bool dowithattach,
				bool doattach, bool dohidevalid,
				bool agedirection, double age, int range, string region, string condition, string type,
				string attachtype, string name, string entry
			)
			{
				Dosearchtype = dotype;
				Dosearchname = doname;
		...
		private static bool TestRange(object o, int range, Map currentmap, Point3D currentloc)
		{
			...
			if (o is Item)
			{
				...
				if (item.Parent != null && item.RootParent != null)
				{
					...
					else if (item.RootParent is Container) loc = ((Container)item.RootParent).Location;
				}
		...
		public static ArrayList Search(SearchCriteria criteria, out string status_str)
		{
			...
			if (!criteria.Dosearchattach)
			{
				...
				if (itemarray != null)
					foreach (Item i in itemarray)
					{
						...
						if (tokunomap != null && i.Map == tokunomap && criteria.Dosearchtok) hasmap = true;
						...
						if (criteria.Dosearchtype &&
						    (i.GetType().IsSubclassOf(targetType) || i.GetType().Equals(targetType))) hastype = true;
						if (criteria.Dosearchtype && !hastype) continue;

						// check for name
						if (criteria.Dosearchname && i.Name != null && criteria.Searchname != null &&
						    i.Name.ToLower().IndexOf(criteria.Searchname.ToLower()) >= 0) hasname = true;
						...
						// check for region
						if (criteria.Dosearchregion && TestRegion(i, criteria.Searchregion)) hasregion = true;
						if (criteria.Dosearchregion && !hasregion) continue;

						// check for attachments
						if (criteria.Dosearchwithattach && TestAttach(i)) hasattach = true;
						...
						if (criteria.Dosearchcondition && criteria.Searchcondition != null)
							...
							hascondition = BaseXmlSpawner.CheckPropertyString(null, i, criteria.Searchcondition, null,
								out status_str);
						...
						if (criteria.Dosearchspawnentry)
						{
							...
							else
							{
								...
								if (i is XmlSpawner)
									...
									foreach (var so in ((XmlSpawner)i).m_SpawnObjects)
										if (criteria.Dosearchspawntype)
										{
											...
											if (so.TypeName != null)
											{
												...
												string typestr = null;
												if (args != null && args.Length > 0) typestr = args[0];
									...
									// search the entries of the spawner
									foreach (var so in ((Spawner)i).SpawnObjects)
									{
										var spawnName = so.SpawnName;
										if (criteria.Dosearchspawntype)
										{
											// search by entry type
											Type type = null;

											if (so != null) type = SpawnerType.GetType(spawnName);
			...
			if (!criteria.Dosearcherr && !criteria.Dosearchattach)
			{
				...
				if (mobilearray != null)
					foreach (Mobile i in mobilearray)
					{
						...
						if (criteria.Dohidevalidint && TestValidInternal(i)) hasvalidhidden = true;
						if (criteria.Dohidevalidint && hasvalidhidden) continue;
			...
			if (criteria.Dohidevalidint)
				foreach (var i in XmlAttach.Values)
					...
					{
						var s = (XmlSaveItem)i;
						if (s.Container != null) ignoreList.Add(s.Container);
					}

			if (criteria.Dosearchattach)
				foreach (var i in XmlAttach.Values)
					// check for type
					if (i != null && !i.Deleted && (targetattachType == null ||
			...
			foreach (SearchEntry se in removelist) newarray.Remove(se);
		...
		public static void XmlFind_OnCommand(CommandEventArgs e)
		{
			...
			if (e.Arguments.Length > 0) typename = e.Arguments[0];
			...
			: base(X, Y)
		{
			...
			if (m_ShowExtension)
			{
				...
				for (var i = 0; i < (int)(MaxEntries / MaxEntriesPerPage); i++)
				...
				AddButton(415 + 25 * (int)(MaxEntries / MaxEntriesPerPage), height - 25, 0x15E1, 0x15E5, 201,
				...
				for (var i = 0; i < MaxEntries; i++)
				{
					...
					var page = (int)(i / MaxEntriesPerPage);
					...
					else if (o is XmlAttachment)
					{
						...
						if (a.AttachedTo is Item)
						{
							...
							if (item.Parent != null)
							{
								...
								else if (item.RootParent is Item) locstr = ((Item)item.RootParent).Location.ToString();
							}
							else
								locstr = item.Location.ToString();
						}

						if (a.Deleted) mapstr = "Deleted";
					}
					...
					if (XmlAttach.HasAttachments(o))
						AddButton(238, 22 * (i % MaxEntriesPerPage) + 35, 2103, 2103, 5000 + i, GumpButtonType.Reply,
							0);
		...
		private void SortFindList()
		{
			if (m_SearchList != null && m_SearchList.Count > 0)
			{
				...
				else if (Sortselect) m_SearchList.Sort(new ListSelectSorter(m_From, Descendingsort));
			}
		}
		...
		private class ListTypeSorter : IComparer
		{
			...
			public int Compare(object e1, object e2)
			{
				...
				else if (y is Mobile) str = ((Mobile)y).GetType().ToString();
				if (str != null)
				{
					var arglist = str.Split('.');
		...
		private class ListNameSorter : IComparer
		{
			...
			public int Compare(object e1, object e2)
			{
				...
				if (x is Item)
					xstr = ((Item)x).Name;
				else if (x is Mobile) xstr = ((Mobile)x).Name;

				if (y is Item)
					ystr = ((Item)y).Name;
				else if (y is Mobile) ystr = ((Mobile)y).Name;
				if (Dsort)
					return String.Compare(ystr, xstr, true);
				else
					return String.Compare(xstr, ystr, true);
		...
		private void ResetList()
		{
			...
			for (var i = 0; i < m_SearchList.Count; i++)
			{
				...
				if (e.Selected)
				{
					...
					if (o is XmlSpawner) ((XmlSpawner)o).DoReset = true;
				}
			}
		}
		...
		private void RespawnList()
		{
			...
			for (var i = 0; i < m_SearchList.Count; i++)
			{
				...
				if (e.Selected)
				{
					...
					if (o is XmlSpawner) ((XmlSpawner)o).DoRespawn = true;
				}
			}
		}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			try
			{
				DisplayFrom = Int32.Parse(tr.Text);
			}
			catch { }
						...
						if (radiostate == 1 && SearchList != null)
							...
							for (var i = 0; i < SearchList.Count; i++)
							{
								...
								if (e.Selected)
								{
									...
									else if (o is Mobile) ((Mobile)o).MoveToWorld(myloc, mymap);
								}
							}
