...
namespace Server.Engines.XmlSpawner2
{
	...
	public abstract class BaseChallengeGame : Item, IChallengeGame
	{
		...
		public override bool Decays => !GameLocked;
		...
		public virtual bool AreInGame(Mobile from)
		{
			...
			if (Participants != null)
				foreach (IChallengeEntry entry in Participants)
				{
					...
					if (entry.Participant == @from) return true;
				}
		...
		public void ClearArena()
		{
			...
			foreach (Mobile p in mlist)
				for (var i = 0; i < 10; i++)
				{
					var x = Location.X + (ArenaSize + i) * (Utility.RandomBool() ? 1 : -1);
					var y = Location.Y + (ArenaSize + i) * (Utility.RandomBool() ? 1 : -1);
					var z = Map.GetAverageZ(x, y);
					var newloc = new Point3D(x, y, z);

					if (XmlSpawner.IsValidMapLocation(newloc, p.Map)) p.MoveToWorld(newloc, p.Map);
				}
		}
		...
		public virtual bool HasEntryFee(Mobile m)
		{
			...
			if (bank != null)
			{
				var goldlist = bank.FindItemsByType(typeof(Gold), true);
		...
		public virtual void Forfeit(Mobile m)
		{
			...
			var a = (XmlPoints)XmlAttach.FindAttachment(m, typeof(XmlPoints));
			if (a != null) a.ChallengeGame = null;
		}
		...
		public static void DoSetupChallenge(Mobile from, int nameindex, Type gametype)
		{
			if (from != null && gametype != null)
			{
				...
				var r = Region.Find(from.Location, from.Map);
		...
		public virtual bool ChallengeBeingCancelled => false;

		public virtual string ChallengeName => Name;

		public virtual ArrayList Organizers => null;

		public virtual bool UseKillDelay => true;

		public virtual bool AllowPoints => false;
		...
		public override void OnDelete()
		{
			...
			if (GameInProgress)
			{
				...
				if (Participants != null)
				{
					...
					// clear all noto
					foreach (IChallengeEntry entry in Participants) RefreshNoto(entry.Participant);
				}
