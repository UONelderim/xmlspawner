	...
	public class XmlEditDialogGump : Gump
	{
		...
		public class XmlConfirmAddGump : Gump
		{
			...
			public XmlConfirmAddGump(Mobile from, object targeted) : base(0, 0)
			{
				...
				else if (targeted is Mobile) name = ((Mobile)targeted).Name;

				if (name == null) name = targeted.GetType().Name;
			...
			bool selectall, bool[] selectionlist, int X, int Y) : base(X, Y)
		{
			...
			if (m_SelectionList == null) m_SelectionList = new bool[MaxEntries];
			...
			else if (m_Dialog.AttachedTo is Mobile) Name = ((Mobile)m_Dialog.AttachedTo).Name;
			if (Name == null && m_Dialog.AttachedTo != null) Name = m_Dialog.AttachedTo.GetType().Name;
			...
			for (var i = 0; i < MaxEntries; i++)
			{
				...
				var s = (XmlDialog.SpeechEntry)m_SearchList[index];
				...
				if (m_SelectionList != null && i < m_SelectionList.Length) sel = m_SelectionList[i];
			...
			if (m_SearchList != null)
			{
				...
				var last = DisplayFrom + MaxEntries < m_SearchList.Count
		...
		{
			if (m_SearchList != null && m_SearchList.Count > 0) m_SearchList.Sort(new ListSorter(false));
		}
		...
		{
			var g = new XmlEditDialogGump(m_From, false, m_Dialog, Selected,
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (m_Dialog != null)
			{
				...
				if (tr != null)
					try
					{
						m_Dialog.ProximityRange = Int32.Parse(tr.Text);
					}
					catch { }
				...
				if (tr != null)
					try
					{
						m_Dialog.SpeechPace = Int32.Parse(tr.Text);
					}
					catch { }
			...
			{
				// entry information
				var entry = (XmlDialog.SpeechEntry)m_SearchList[Selected + DisplayFrom];

				tr = info.GetTextEntry(200); // entry number
				if (tr != null)
					try
					{
						entry.EntryNumber = Int32.Parse(tr.Text);
					}
					catch { }
				...
				if (tr != null)
					try
					{
						entry.PrePause = Int32.Parse(tr.Text);
					}
					catch { }
				...
				if (tr != null)
					try
					{
						entry.Pause = Int32.Parse(tr.Text);
					}
					catch { }
				...
				if (tr != null)
					try
					{
						entry.SpeechHue = Int32.Parse(tr.Text);
					}
					catch { }
				...
				default:
				{
					...
					else if (info.ButtonID == 3999)
					{
						...
						if (m_SelectionList != null && m_SearchList != null && !SelectAll)
							for (var i = 0; i < MaxEntries; i++)
								...
								{
									// only toggle the selection list entries for things that actually have entries
									if (m_SearchList.Count - DisplayFrom > i) m_SelectionList[i] = !m_SelectionList[i];
								}
								else
									break;
					}
					...
					else if (info.ButtonID >= 5000 && info.ButtonID < 5100)
					{
						...
						var book = new XmlTextEntryBook(0, String.Empty, m_Dialog.Name, 20, true,
							new XmlTextEntryBookCallback(ProcessXmlEditBookEntry), args);

						if (m_Dialog.m_TextEntryBook == null) m_Dialog.m_TextEntryBook = new ArrayList();
				...
				switch (info.ButtonID)
				{
					default:
					{
						if (radiostate == 1 && SearchList != null && SelectedList != null)
						{
							...
							foreach (var o in dlist) SearchList.Remove(o);
