	...
	public class MobFactionRegionStone : Item
	{
		...
		public Point3D EjectLocation
		{
			...
			set
			{
				...
				if (m_MobFactionRegion != null) m_MobFactionRegion.EjectLocation = m_EjectLocation;
				//RefreshRegions();
			}
		}
		...
		public Map EjectMap
		{
			...
			{
				m_EjectMap = value;
				if (m_MobFactionRegion != null) m_MobFactionRegion.EjectMap = m_EjectMap;
				//RefreshRegions();
			}
		}

		[CommandProperty(AccessLevel.GameMaster)]
		public Rectangle3D[] MobFactionArea => m_MobFactionArea;

		[CommandProperty(AccessLevel.GameMaster)]
		public string MobFactionRegionName
		{
			get => m_MobFactionRegionName;
			set
			{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public MobFactionRegion MobFactionRegion => m_MobFactionRegion;
		...
		public XmlMobFactions.GroupTypes FactionType
		{
			...
			{
				m_FactionType = value;
				if (m_MobFactionRegion != null) m_MobFactionRegion.FactionType = m_FactionType;
		...
		public int FactionLevel
		{
			...
			set
			{
				...
				if (m_MobFactionRegion != null) m_MobFactionRegion.FactionLevel = m_FactionLevel;
		...
		{
			if (m_MobFactionRegion != null) m_MobFactionRegion.Unregister();
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			if (m_MobFactionRegion != null && m_MobFactionRegion.Area != null && m_MobFactionRegion.Area.Length > 0)
			{
				...
				for (var i = 0; i < m_MobFactionRegion.Area.Length; i++)
				{
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				case 1:
				{
					...
					var ejectmap = reader.ReadString();
					try
					{
					...
					var factiontype = reader.ReadString();
					try
					{
						m_FactionType =
				...
				case 0:
				{
					...
					if (count > 0)
					{
						...
						if (version < 1)
						{
							...
							for (var i = 0; i < count; i++) area[i] = reader.ReadRect2D();
			...
			object state)
		{
			...
			var s = state as MobFactionRegionStone;
