...
namespace Server.Gumps
{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				default:
				{
					...
					else if (m_From != null && m_Item != null && !m_Item.Deleted) m_From.AddToBackpack(m_Item);
	...
	public class XmlSimpleGump : Gump
	{
			...
			bool scrollbar)
		{
			...
			{
				var cliloc = 0;
				if (Int32.TryParse(text.Substring(1, text.Length - 1), out cliloc))
					AddHtmlLocalized(x, y, width, height, cliloc, color, background, scrollbar);
			}
			else
				try
				{
					var colorstring = String.Format("{0:X}", color);
			...
			BaseXmlSpawner.KeywordTag tag, Mobile from, XmlGumpCallback gumpcallback) : base(0, 0)
		{
			...
			if (gumptype == 5)
			{
				...
				var args = maintext.Split(',');
				if (args != null && args.Length > 1)
				{
					Int32.TryParse(args[0].Trim(), out width);
					Int32.TryParse(args[1].Trim(), out height);
				}
			}
			...
			else if (gumptype == 4)
			{
				...
				for (var i = 0; i < gumpSelections.Count; i++)
				{
					var y = 360 + i * 40;
				...
				for (var i = 0; i < gumpSelections.Count; i++)
				{
					var selection = gumpSelections[i].Selection;
					var response = gumpSelections[i].Response;
					...
					if (selection != null) args = selection.Split(',');

					// process the gumpitem specifications
					if (args.Length > 1)
					{
						for (var j = 0; j < args.Length; j++) args[j] = args[j].Trim();
						...
						if (args[0].ToLower() == "button")
						{
							...
							if (args.Length > 3)
							{
								if (args[1].StartsWith("0x"))
									Int32.TryParse(args[1].Substring(2), NumberStyles.HexNumber,
										CultureInfo.InvariantCulture, out gumpid);
								else
									Int32.TryParse(args[1], out gumpid);
								Int32.TryParse(args[2], out gx);
								Int32.TryParse(args[3], out gy);
						...
						else if (args[0].ToLower() == "label")
						{
							...
							if (args.Length > 3)
							{
								Int32.TryParse(args[1], out gx);
								Int32.TryParse(args[2], out gy);
							...
							color = 0x384;
							if (args.Length > 4) Int32.TryParse(args[4], out color);
						...
						else if (args[0].ToLower() == "html")
						{
							...
							if (args.Length > 5)
							{
								Int32.TryParse(args[1].Trim(), out gx);
								Int32.TryParse(args[2].Trim(), out gy);
								Int32.TryParse(args[3].Trim(), out gwidth);
								Int32.TryParse(args[4].Trim(), out gheight);
								if (args.Length > 6 && args[5].StartsWith("0x") && args[5].Trim().Length == 8)
								{
									if (!Int32.TryParse(args[5].Trim().Substring(2), NumberStyles.HexNumber,
						...
						else if (args[0].ToLower() == "textentry")
						{
							...
							if (args.Length > 5) label = args[5];

							// set the default textentry color
							color = 0x384;
							if (args.Length > 6) Int32.TryParse(args[6], out color);
						...
						else if (args[0].ToLower() == "radio")
						{
							...
							if (args.Length > 4)
							{
								Int32.TryParse(args[1].Trim(), out gumpid1);
								Int32.TryParse(args[2].Trim(), out gumpid2);
								Int32.TryParse(args[3].Trim(), out gx);
								Int32.TryParse(args[4].Trim(), out gy);
							}

							var initial = false;
							if (args.Length > 5) Boolean.TryParse(args[5], out initial);
						...
						else if (args[0].ToLower() == "imagetiled")
						{
							...
							if (args.Length > 5)
							{
								...
								Int32.TryParse(args[2], out gx);
								Int32.TryParse(args[3], out gy);
								Int32.TryParse(args[4], out gwidth);
								Int32.TryParse(args[5], out gheight);
				...
				if (info.ButtonID == 0)
					...
					switch (m_gumptype)
					{
						...
						case 5:
							...
							var buttonresponse = String.Empty;
							var radioresponse = String.Empty;
							var textresponse = String.Empty;

							if (info.ButtonID >= 1000)
							{
								var select = info.ButtonID - 1000;
							...
							if (info.Switches != null && info.Switches.Length > 0)
							{
								var radiostate = info.Switches[0];
							...
							for (var j = 0; j < gumpSelections.Count; j++)
								if (gumpSelections[j].GumpItemType == 1)
									try
									{
										var te = info.GetTextEntry(j);
										if (te != null && te.Text.Length > 0) textresponse += te.Text + " ";
									}
									catch { }

							// build the composite reponse string
							string responsestring = null;
							if (buttonresponse != null && buttonresponse.Length > 0) responsestring = buttonresponse;
							if (radioresponse != null && radioresponse.Length > 0)
								responsestring += " " + radioresponse;
							if (textresponse != null && textresponse.Length > 0) responsestring += " " + textresponse;
