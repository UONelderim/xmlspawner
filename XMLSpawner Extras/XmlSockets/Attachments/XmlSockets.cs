...
namespace Server.Engines.XmlSpawner2
{
	public class XmlSockets : XmlAttachment
	{
		...
		{
			get => m_SocketOccupants;
			set => m_SocketOccupants = value;
		}
		...
		public int NFree
		{
			get
			{
				...
				{
					var count = 0;
					for (var i = 0; i < m_SocketOccupants.Count; i++)
						if (m_SocketOccupants[i] != null)
							count++;
		...
		public class LimitSocketsCommand : BaseCommand
		{
				...
				for (var i = 0; i < list.Count; ++i)
				{
					...
					if (s != null && s.SocketOccupants != null && s.SocketOccupants.Count > m_maxsockets)
					{
						...
						while (found && nchange <= maxsock)
						{
							...
							for (var j = 0; j < s.SocketOccupants.Count; j++)
							{
								var so = s.SocketOccupants[j] as SocketOccupant;
								if (so != null)
								{
									nchange++;

									// recover the old version of the augment
									var augment = s.RecoverAugmentation(null, targetobject, j);
						...
						for (var k = 0; k < m_maxsockets; k++) s.SocketOccupants.Add(null);
					}
				}
			...
			public override bool ValidateArgs(BaseCommandImplementor impl, CommandEventArgs e)
			{
				if (e.Arguments.Length >= 1)
				{
					try
					{
						m_maxsockets = Int32.Parse(e.GetString(0));
					}
					catch
					{
		...
		public class UpgradeAugmentCommand : BaseCommand
		{
			...
			public override bool ValidateArgs(BaseCommandImplementor impl, CommandEventArgs e)
			{
				if (e.Arguments.Length >= 2)
				{
					...
					try
					{
						m_oldversion = Int32.Parse(e.GetString(1));
					}
					catch { }

					if (m_oldversion != -1) return true;
				}
		...
		public XmlSockets(int nsockets)
		{
			...
			for (var i = 0; i < nsockets; i++) m_SocketOccupants.Add(null);
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			else
			{
				...
				for (var i = 0; i < m_SocketOccupants.Count; i++)
				{
					var s = (SocketOccupant)m_SocketOccupants[i];
					if (s != null)
					{
						var typename = s.OccupantType.ToString();
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (typename != null)
				{
					...
					var augmentversion = 0;
					if (version > 1)
						// version 2
						augmentversion = reader.ReadInt();
					var id = reader.ReadInt();
					var desc = reader.ReadString();
					var xoff = reader.ReadInt();
					var yoff = reader.ReadInt();
					var hue = reader.ReadInt();
					var use = reader.ReadBool();
			...
			double chance2, double chance1, SkillName skillname, double minskill, Type resource, int quantity)
		{
			if (Utility.Random(1000) < (int)(dropchance * 10))
			{
				...
				var socklevel = Utility.Random(1000);
		...
		public static int ComputeSuccessChance(Mobile from, int nsockets, double difficulty, SkillName requiredSkill)
		{
			...
			var successchance = (int)(2 * (skill - difficulty) + 15 - (nsockets - 1) * 13);
			...
			double difficulty2, SkillName requiredSkill2, Type resource, int quantity)
		{
			...
			var s = XmlAttach.FindAttachment(target, typeof(XmlSockets)) as XmlSockets;
		...
		public static bool Augment(Mobile from, object parent, XmlSockets sock, int socketnum, IXmlSocketAugmentation a)
		{
			...
			{
				var occ = new SocketOccupant(a.GetType(), a.Version, a.Icon, a.IconXOffset, a.IconYOffset, a.IconHue,
					a.UseGumpArt,
					String.Format("{0}\n{1}\n{2} Socket(s)", a.Name, a.OnIdentify(null), a.SocketsRequired));

				var remaining = a.SocketsRequired;
				...
				if (remaining > 0)
					// find a free socket
					for (var i = 0; i < sock.SocketOccupants.Count; i++)
					{
		...
		public BaseSocketAugmentation RecoverAugmentation(Mobile from, object o, int index)
		{
			...
			var s = SocketOccupants[index] as SocketOccupant;

			if (s == null) return null;

			var t = s.OccupantType;
			if (t != null)
				try
				{
			...
			if (augment != null)
			{
				    ...
				    augment.RecoverableSockets(s.AugmentationVersion) > 0)
				{
					...
					var count = augment.RecoverableSockets(s.AugmentationVersion);
					for (var i = 0; i < SocketOccupants.Count; i++)
		...
		{
			var nfilled = NSockets - NFree;

			if (nfilled > 0)
			{
				// pick a random augmentation from a filled socket
				var rindex = Utility.Random(nfilled);
				var count = 0;
				for (var i = 0; i < SocketOccupants.Count; i++)
					if (SocketOccupants[i] != null)
					{
						if (count == rindex)
						{
							...
							var augment = RecoverAugmentation(@from, o, i);
		...
		private class AddAugmentationToSocket : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				{
					var a = targeted as IXmlSocketAugmentation;
					...
					if (m_parent is Item && ((Item)m_parent).Parent == null || m_parent is Mobile)
					{
						...
						else if (m_parent is Mobile) loc = ((Mobile)m_parent).Location;
		...
		public class AddSocketToTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				var maxSockets = DefaultMaxSockets;
				var difficulty = DefaultSocketDifficulty;
				var skillname = DefaultSocketSkill;
				var difficulty2 = DefaultSocketDifficulty;
				var skillname2 = DefaultSocketSkill;
				var resource = DefaultSocketResource;
				var quantity = DefaultSocketResourceQuantity;

				// see if this has an XmlSocketable attachment that might impose socketing restrictions
				var s = XmlAttach.FindAttachment(targeted, typeof(XmlSocketable)) as XmlSocketable;
				...
				if (maxSockets < 0 && m_extrasockets > 0) maxSockets = 0;
		...
		public class RecoverAugmentationFromTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				var s = XmlAttach.FindAttachment(targeted, typeof(XmlSockets)) as XmlSockets;
				...
				var augment = s.RecoverRandomAugmentation(from, targeted);
		...
		private class SocketsGump : Gump
		{
			...
			public SocketsGump(Mobile from, XmlSockets a) : base(0, 0)
			{
				...
				if (a.SocketOccupants != null) socketcount = a.SocketOccupants.Count;
				...
				{
					var item = m_attachment.AttachedTo as Item;

					label = item.Name;

					if (item.Name == null) label = item.ItemData.Name;
				}
				else if (m_attachment.AttachedTo is Mobile)
				{
					var m = m_attachment.AttachedTo as Mobile;
				...
				{
					var s = (SocketOccupant)m_attachment.SocketOccupants[i];
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				for (var i = 0; i < m_attachment.SocketOccupants.Count; i++)
				{
					var s = (SocketOccupant)m_attachment.SocketOccupants[i];
