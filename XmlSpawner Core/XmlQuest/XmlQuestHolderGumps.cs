	...
	public class XmlQuestStatusGump : Gump
	{
			...
			string descriptionstr)
		{
			if (objectivestr != null && objectivestr.Length > 0)
			{
				...
				if (arglist.Length > 0)
					switch (arglist[0])
					{
						...
						case "KILLNAMED":
							...
							if (arglist.Length > 1) name = arglist[1];
				...
				if (foundkill)
				{
					...
					try
					{
						killed = Int32.Parse(statestr);
					}
					catch { }

					var remaining = targetcount - killed;
				...
				else if (foundescort)
				{
					...
					try
					{
						escorted = Int32.Parse(statestr);
					}
					catch { }

					var remaining = targetcount - escorted;
				...
				else if (foundcollect)
				{
					...
					try
					{
						collected = Int32.Parse(statestr);
					}
					catch { }
			...
			: base(X, Y)
		{
			...
			if (screen == 1)
			{
				...
				if (questitem.Journal != null && questitem.Journal.Count > 0)
				{
					...
					for (var i = 0; i < questitem.Journal.Count; i++)
					{
						journaltext += "<u>";
			...
			else
			{
				...
				else if (questitem.AlreadyDone)
				{
					...
					{
						var a = XmlAttach.FindAttachments(questitem.Owner, typeof(XmlQuestAttachment), questitem.Name);
						if (a != null && a.Count > 0)
		...
		public class JournalEntryGump : Gump
		{
				...
				: base(X, Y)
			{
				...
				var entrynumber = 0;
				if (questitem.Journal != null && questitem.Journal.Count > 0) entrynumber = questitem.Journal.Count;
				var m_entryid = "Personal entry #" + entrynumber;
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				if (update_entry)
				{
					...
					if (entry != null) entrytext = entry.Text;

					entry = info.GetTextEntry(1);
					if (entry != null) entryid = entry.Text;
