...
namespace Server.Items
{
	public class DeathBall : Item
	{
		...
		public override bool CheckLift(Mobile from, Item item, ref LRReason reject)
		{
			...
			if (from != null && from.AccessLevel > AccessLevel.Player) return base.CheckLift(@from, item, ref reject);
	...
	public class DeathBallGauntlet : BaseChallengeGame
	{
		...
		public DeathBall Ball => m_DeathBall;
		...
		{
			Mobile owner = null;
			if (m_DeathBall != null && m_DeathBall.RootParent is Mobile) owner = m_DeathBall.RootParent as Mobile;

			// only score if one player is carrying the ball
			if (owner != null)
			{
				var entry = GetParticipant(owner);
		...
		public override void OnPlayerKilled(Mobile killer, Mobile killed)
		{
			...
			{
				var owner = ((Corpse)m_DeathBall.RootParent).Owner;
				if (owner == killed)
				{
