...
namespace Server.Mobiles
{
	public class XmlGetAttGump : Gump
	{
		...
		{
			status_str = null;
			var newarray = new List<XmlAttachment>();
			...
			var attachments = XmlAttach.FindAttachments(target);
			...
			if (attachments != null)
				foreach (var i in attachments)
				{
					...
					if (Dosearchname && i.Name != null && Searchname != null &&
					    i.Name.ToLower().IndexOf(Searchname.ToLower()) >= 0) hasname = true;
			...
			bool selectall, bool[] selectionlist, int X, int Y) : base(X, Y)
		{
			...
			else if (targeted is Mobile) tnamestr = ((Mobile)targeted).Name;
			...
			for (var i = 0;
			     i < (int)(MaxEntries / MaxEntriesPerPage);
			     i++) //AddButton( 38+i*30, 365, 2206, 2206, 0, GumpButtonType.Page, 1+i );
			...
			for (var i = 0; i < MaxEntries; i++)
			{
				...
				if (i % MaxEntriesPerPage == 0) AddPage(page + 1);
				...
				var o = (object)m_SearchList[index];
		...
		private void SortFindList()
		{
			if (m_SearchList != null && m_SearchList.Count > 0)
			{
				...
				else if (Sortname) m_SearchList.Sort(new ListNameSorter(Descendingsort));
			}
		}
				...
				default:
				{
					...
					if (info.ButtonID == 3998)
					{
						...
						if (m_SelectionList != null)
							for (var i = 0; i < MaxEntries; i++)
								if (i < m_SelectionList
									    .Length) // only toggle the selection list entries for things that actually have entries
									m_SelectionList[i] = SelectAll;
								else
									break;
					}

					if (info.ButtonID == 3999) // dont allow individual selection with the selectall button selected
					...
					if (info.ButtonID >= 4000 && info.ButtonID < 4000 + MaxEntries)
					{
						...
						// dont allow individual selection with the selectall button selected
						if (m_SelectionList != null && i >= 0 && i < m_SelectionList.Length &&
						    !SelectAll) // only toggle the selection list entries for things that actually have entries
					...
					if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries)
					{
						...
						if (m_SelectionList != null && i >= 0 && i < m_SelectionList.Length &&
							...
							if (m_SearchList != null && m_SearchList.Count - DisplayFrom > i)
							{
								var a = m_SearchList[i + DisplayFrom];
								if (a != null) state.Mobile.SendMessage(a.OnIdentify(state.Mobile));
							}
					}
		...
		public class XmlConfirmDeleteGump : Gump
		{
			...
			public XmlConfirmDeleteGump(Mobile from, object target, List<XmlAttachment> searchlist, bool[] selectedlist,
