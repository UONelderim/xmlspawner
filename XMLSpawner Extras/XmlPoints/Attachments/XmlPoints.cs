...
namespace Server.Engines.XmlSpawner2
{
	public class XmlPoints : XmlAttachment
	{
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime LastKill => m_LastKill;

		[CommandProperty(AccessLevel.GameMaster)]
		public DateTime LastDeath => m_LastDeath;

		public bool HasChallenge => m_Challenger != null && !m_Challenger.Deleted ||
		...
		public static bool DuelLocationAvailable(DuelLocationEntry duelloc)
		{
			...
			var duelrange = duelloc.DuelRange;

			if (duelloc.DuelRange <= 0) duelrange = 16;

			foreach (var m in duelloc.DuelMap.GetMobilesInRange(duelloc.DuelLocation, duelrange))
				if (m.Player)
					return false;
		...
		public static void AddText(LanguageType t, int index, string text)
		{
			...
			if (TextHash[tindex] == null) TextHash[tindex] = new Hashtable();

			var h = TextHash[tindex];
		...
		public static string SystemText(int index)
		{
			...
			var h = TextHash[(int)m_SystemLanguage];

			if (h == null || !h.Contains(index)) return String.Format("??Entry {0}??", index);
		...
		public string Text(int index)
		{
			...
			var h = TextHash[(int)m_CurrentLanguage];
		...
		public static string GetText(Mobile from, int msgindex)
		{
			...
			var a = (XmlPoints)XmlAttach.FindAttachment(from, typeof(XmlPoints));
		...
		public void SendColorText(int color, int msgindex)
		{
			...
			if (AttachedTo is Mobile) @from = (Mobile)AttachedTo;
			// go through the participant list and send all participants the message
			if (from == null) return;
		...
		public static void SendColorText(Mobile from, int color, int msgindex)
		{
			...
			if (a != null) @from.SendMessage(color, a.Text(msgindex));
		}
		...
		public static void SendColorText(Mobile from, int color, int msgindex, object arg)
		{
			...
			if (a != null) @from.SendMessage(color, String.Format(a.Text(msgindex), arg));
		}
		...
		public static void SendColorText(Mobile from, int color, int msgindex, object arg, object arg2)
		{
			...
			if (a != null) @from.SendMessage(color, String.Format(a.Text(msgindex), arg, arg2));
		}
		...
		private static void UpdateRanking(Mobile m, XmlPoints attachment)
		{
			...
			if (!found) RankList.Add(new RankEntry(m, attachment));
		...
		public static int GetCredits(Mobile m)
		{
			...
			var x = XmlAttach.FindAttachment(m, typeof(XmlPoints)) as XmlPoints;
			if (x != null) val = x.Credits;
		...
		new public static void Initialize()
		{
			...
			foreach (var i in World.Items.Values)
				if (i is BaseChallengeGame && !((BaseChallengeGame)i).GameCompleted)
				{
					...
					var r = Region.Find(i.Location, i.Map);
		...
		{
			var dirname = Path.Combine(m_LeaderboardSaveDirectory, filename);
			...
			for (var i = 0; i < RankList.Count; i++)
			{
				...
				var r = RankList[i] as RankEntry;
				var a = r.PointsAttachment;
		...
		public static string HtmlSpecialEncoding(string text)
		{
			...
			for (var i = 0; i < text.Length; i++) encoded += String.Format("&#{0};", (int)text[i]);
			...
			if (e.Arguments.Length > 0)
			{
				...
				if (e.Arguments[0].ToLower() != "off")
				{
					...
					if (e.Arguments.Length > 2)
						try
						{
							m_LeaderboardSaveRanks = Int32.Parse(e.Arguments[2]);
						}
						catch { }
		...
		{
			var a = (XmlPoints)XmlAttach.FindAttachment(e.Mobile, typeof(XmlPoints));
		...
		public static void CheckPoints_OnCommand(CommandEventArgs e)
		{
			...
			var x = XmlAttach.FindAttachment(e.Mobile, typeof(XmlPoints)) as XmlPoints;
			if (x != null) msg = x.OnIdentify(e.Mobile);
			if (msg != null)
			...
			if (x != null)
			{
				if (e.Arguments.Length > 0)
				{
					...
					if (e.Arguments[0] == "si" || e.Arguments[0] == "ok" || e.Arguments[0] == "vero")
						...
						Boolean.TryParse(e.Arguments[0], out result);
		...
		{
			if (e.Arguments.Length > 0) Boolean.TryParse(e.Arguments[0], out m_SystemBroadcast);
		...
		public static bool AreChallengers(Mobile from, Mobile target)
		{
			...
			var afrom = (XmlPoints)XmlAttach.FindAttachment(from, typeof(XmlPoints));

			if (afrom == null || afrom.Deleted)
				return false;

			var atarget = (XmlPoints)XmlAttach.FindAttachment(target, typeof(XmlPoints));
		...
		public static bool AreInAnyGame(Mobile target)
		{
			...
			if (atarget != null && !atarget.Deleted && atarget.m_ChallengeGame != null &&
			    !atarget.m_ChallengeGame.Deleted) return atarget.m_ChallengeGame.AreInGame(target);
		...
		public static bool InsuranceIsFree(Mobile from, Mobile awardto)
		{
			...
			var atarget = (XmlPoints)XmlAttach.FindAttachment(awardto, typeof(XmlPoints));
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is Mobile && ((Mobile)targeted).Player)
				{
					...
					if (a != null && !a.Deleted)
					{
						...
						var xa = (XmlPoints)XmlAttach.FindAttachment(pm, typeof(XmlPoints));
		...
		public class CancelTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				var xa = (XmlPoints)XmlAttach.FindAttachment(m_attachment.Challenger, typeof(XmlPoints));
		...
		public static void AddAllPoints_OnCommand(CommandEventArgs e)
		{
			...
			foreach (var m in World.Mobiles.Values)
				if (m.Player)
				{
					...
					var x = XmlAttach.FindAttachment(m, typeof(XmlPoints));
					if (x == null)
					{
						x = new XmlPoints();
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				...
				case 5:
				{
					...
					if (remaining > TimeSpan.Zero) DoTimer(remaining);
					goto case 4;
				}
				case 4:
				{
				...
				case 0:
				{
					...
					for (var i = 0; i < count; i++)
					{
						...
						var t = reader.ReadDateTime();
						if (m != null && !m.Deleted) KillList.Add(new KillEntry(m, t));
					}

					// get the owner of this in order to rebuild the rankings
					var killer = reader.ReadMobile();

					// rebuild the ranking list
					// if they have never made a kill, then dont rank
					if (killer != null && m_Kills > 0) UpdateRanking(killer, this);
					break;
				}
			}
		}
		...
		public override bool HandlesOnKill => true;
		...
		public override void OnKill(Mobile killed, Mobile killer)
		{
			...
			if (ChallengeGame != null && !ChallengeGame.Deleted) ChallengeGame.OnKillPlayer(killer, killed);
			...
			if (killed == m_Challenger || killer == m_Challenger) m_Challenger = null;
			...
			var x = XmlAttach.FindAttachment(killed, typeof(XmlPoints)) as XmlPoints;

			if (x != null) killedpoints = x.Points;
		...
		public void ReportPointLoss_Callback(object state)
		{
			...
			if (m != null) SendText(m, 100217, points, name); // "You lost {0} point(s) for being killed by {1}"
		}
		...
		public static void AutoRes_Callback(object state)
		{
			...
			if (m != null)
			{
				...
				{
					m.Hits = m.HitsMax;
					m.Mana = Int32.MaxValue;
					m.Stam = Int32.MaxValue;
				}
			}
		}
		...
		public static void Return_Callback(object state)
		{
			...
			if (killer != null && killed != null && map != null && map != Map.Internal)
			{
				...
				if (killer.Corpse != null) killer.Corpse.MoveToWorld(loc, map);

				// port the killed and corpse
				killed.PlaySound(0x214);
				killed.FixedEffect(0x376A, 10, 16);
				killed.MoveToWorld(loc, map);
				if (killed.Corpse != null) killed.Corpse.MoveToWorld(loc, map);
			}
		}
		...
		public bool CanAffectPoints(Mobile from, Mobile killer, Mobile killed, bool assumechallenge)
		{
			...
			var norestriction = UnrestrictedChallenges;
			...
			var a = this;
			if (from != killer) a = (XmlPoints)XmlAttach.FindAttachment(killer, typeof(XmlPoints));
			if (a != null)
			{
				a.RefreshKillList();
		...
		{
			// and remove the challenger from the aggressor list so that the res noto is not affected
			var klist = target.Aggressors;
			if (klist != null && klist.Count > 0)
				for (var j = 0; j < klist.Count; j++)
		...
		public override void OnKilled(Mobile killed, Mobile killer)
		{
			...
			if (killed == m_Challenger || killer == m_Challenger || AreInSameGame(killer, killed))
			{
				...
				{
					var klist = ((Corpse)killed.Corpse).Aggressors;
					if (klist != null && klist.Count > 0)
						for (var j = 0; j < klist.Count; j++)
							if (klist[j] == killer)
							{
			...
			if (ChallengeGame != null && !ChallengeGame.Deleted && ChallengeGame.GetParticipant(killer) != null &&
			    ChallengeGame.GetParticipant(killed) != null) ChallengeGame.OnPlayerKilled(killer, killed);
			...
			var xp = XmlAttach.FindAttachment(killer, typeof(XmlPoints)) as XmlPoints;
		...
		public override string OnIdentify(Mobile from)
		{
			...
			var val = GetRanking((Mobile)AttachedTo);
		...
		*/
			...
			public TopPlayersGump(XmlPoints attachment) : base(0, 0)
			{
				...
				for (var i = 0; i < RankList.Count; i++)
				{
					...
					if (r.Killer != null && !r.Killer.Deleted && r.Rank > 0 && a != null && !a.Deleted)
					{
						...
						if (label == null) label = attachment.Text(200253); // "just changed"
		...
		private class IssueChallengeGump : Gump
		{
			...
			public IssueChallengeGump(Mobile from, Mobile target) : base(0, 0)
			{
				...
				var locsize = DuelLocations.Length;

				if (!TeleportOnDuel) locsize = 0;

				var height = 170 + locsize * 30;
				...
				if (TeleportOnDuel && SpellHelper.CheckTravel(from.Map, from.Location, TravelCheckType.RecallFrom) &&
					...
					for (var i = 0; i < DuelLocations.Length; i++)
					{
						// check availability

						if (!DuelLocationAvailable(DuelLocations[i]))
							texthue = 33;
						else
							texthue = 0;
						AddLabel(55, y, texthue, DuelLocations[i].Name);
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				var radiostate = -1;
				if (info.Switches.Length > 0) radiostate = info.Switches[0];
				...
				switch (info.ButtonID)
				{
					default:
					{
						if (radiostate == 1)
						{
							...
							var a = (XmlPoints)XmlAttach.FindAttachment(m_From, typeof(XmlPoints));
							...
							var ta = (XmlPoints)XmlAttach.FindAttachment(m_Target, typeof(XmlPoints));
							...
							if (a != null && !a.Deleted) a.Challenger = m_Target;

							// assign the challenger field on the target points attachment
							if (ta != null && !ta.Deleted) ta.Challenger = m_From;
							...
							{
								var duelloc = m_DuelLocation.DuelLocation;
								var duelmap = m_DuelLocation.DuelMap;
