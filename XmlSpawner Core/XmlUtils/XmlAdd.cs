...
namespace Server.Mobiles
{
			...
			if (DefaultEntryList != null)
				for (var i = 0; i < DefaultEntryList.Count; i++)
				{
					...
					if (entry != null && String.Compare(entry.PlayerName, name, true) == 0 &&
					    String.Compare(entry.AccountName, account, true) == 0) return entry;
				}
	...
	public class XmlAddGump : Gump
	{
		...
		private string NameListToString()
		{
			...
			for (var i = 0; i < defs.NameList.Length; i++) sb.AppendFormat(":{0}", defs.NameList[i]);
			return sb.ToString();
		}
		...
		private string SelectionListToString()
		{
			...
			for (var i = 0; i < defs.SelectionList.Length; i++) sb.AppendFormat(":{0}", defs.SelectionList[i] ? 1 : 0);
			return sb.ToString();
		}
		...
		public XmlAddGump(Mobile from, Point3D startloc, Map startmap) : this(from, startloc, startmap, true, false,
			StartingXoffset, 0)
		{
		}
			...
			int gumpy) : base(gumpx, gumpy)
		{
			...
			if (defs.SelectionList == null) defs.SelectionList = new bool[MaxEntries];
			...
			if (defs.ShowExtension)
			{
				...
				{
					var xpos = (int)(i / MaxEntriesPerColumn) * 155;
					...
					var sel = false;
					if (defs.SelectionList != null && i < defs.SelectionList.Length) sel = defs.SelectionList[i];
					var texthue = 0;
					if (sel) texthue = 68;
					string namestr = null;
		...
		{
			private XmlSpawnerDefaults.DefaultEntry defs;
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (defs.AutoNumber) sname = String.Format("{0}#{1}", defs.SpawnerName, defs.AutoNumberValue);
				...
				var spawner = new XmlSpawner(SpawnId, from.Location.X, from.Location.Y, 0, 0, sname, maxcount,
					...
					defs.DespawnTime, defs.SkillTrigger, defs.SmartSpawning, null);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.HomeRange = Int32.Parse(tr.Text); }
				catch { }

			tr = info.GetTextEntry(108); // SpawnRange
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.SpawnRange = Int32.Parse(tr.Text); }
				catch { }

			tr = info.GetTextEntry(109); // ProximityRange
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.ProximityRange = Int32.Parse(tr.Text); }
				catch { }

			tr = info.GetTextEntry(110); // Team
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.Team = Int32.Parse(tr.Text); }
				catch { }
			...
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.ProximitySound = Int32.Parse(tr.Text); }
				catch { }

			tr = info.GetTextEntry(113); // Kill reset
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.KillReset = Int32.Parse(tr.Text); }
				catch { }
			...
			if (tr != null && tr.Text != null && tr.Text.Length > 0)
				try { defs.AutoNumberValue = Int32.Parse(tr.Text); }
				catch { }
			...
			if (defs.ShowExtension)
				for (var i = 0; i < MaxEntries; i++)
				{
					...
					if (defs.NameList != null && i < defs.NameList.Length && tr != null) defs.NameList[i] = tr.Text;
				}
		...
		private class XmlAddOptionsGump : Gump
		{
			...
			public XmlAddOptionsGump(Mobile from, XmlSpawner lastSpawner) : base(0, 0)
			{
				...
				var acct = from.Account as Account;
				XmlSpawnerDefaults.DefaultEntry defs = null;
				if (acct != null) defs = XmlSpawnerDefaults.GetDefaults(acct.ToString(), @from.Name);
