	...
	public class XmlQuestBookGump : Gump
	{
		...
		public XmlQuestBookGump(PlayerMobile from, XmlQuestBook book, int page, ArrayList list) : base(12, 24)
		{
			...
			if (list == null)
			{
				...
				var questitems = book.FindItemsByType(typeof(IXmlQuest));
				...
				for (var i = 0; i < nquests; ++i) list.Add(questitems[i]);
			}
			...
			for (var i = index; i < index + count && i >= 0 && i < list.Count; ++i)
			{
				...
				if (obj is IXmlQuest)
				{
					...
					if (e.IsValid)
					{
						...
						if (e.ExpirationString.IndexOf("Expires in") >= 0) substring = e.ExpirationString.Substring(11);
						AddLabel(400 - xoffset, y, color, (string)substring);
					}
					else
						AddLabel(400 - xoffset, y, color, "No longer valid");

					if (e.PartyEnabled) AddLabel(270 - xoffset, y, color, "Party");
