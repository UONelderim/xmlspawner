...
namespace Server.Items
{
	public class BaseTransmutationContainer : Container
	{
		...
		public void ConsumeAll()
		{
			...
			foreach (var i in Items) dlist.Add(i);
			foreach (Item i in dlist) i.Delete();
		}
		...
		{
			var diddrop = base.OnDragDrop(from, dropped);
		...
		{
			var diddrop = base.OnDragDropInto(from, item, p);
		...
		public void Transmute(Mobile from)
		{
			...
			foreach (Recipe r in AllRecipes)
			{
				...
				var quantity = new int[r.Quantity.Length];
					...
					for (var j = 0; j < r.Ingredients.Length; j++)
					{
						                                 ...
						                                 rt.IsInterface && ContainsInterface(it.GetInterfaces(), rt)))
						{
							...
							if (r.PropertyTests != null && r.PropertyTests[j] != null &&
							    !BaseXmlSpawner.CheckPropertyString(null, i, r.PropertyTests[j], null, out status_str))
		...
		private class TransmuteGump : Gump
		{
			...
			public TransmuteGump(Mobile from, BaseTransmutationContainer target, Item lifted) : base(0, 0)
			{
				...
				if (target.Items != null) count = target.Items.Count;

				var displayeditems = count;
				if (lifted != null && count > 0) displayeditems = count - 1;

				var height = 160 + LineSpacing * displayeditems;
				var width = 350;
				...
				for (var i = 0; i < count; i++)
					...
					{
						var item = target.Items[i] as Item;
