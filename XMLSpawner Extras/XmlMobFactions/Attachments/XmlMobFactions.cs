...
namespace Server.Engines.XmlSpawner2
{
	public class XmlMobFactions : XmlAttachment
	{
		...
		public static Group[] KillGroups => m_KillGroups;
		...
		new public static void Initialize()
		{
			...
			var PlayerGroup = new Group(GroupTypes.Player);
			var UndeadGroup = new Group(GroupTypes.Undead);
			var HumanoidGroup = new Group(GroupTypes.Humanoid);
			var ArachnidGroup = new Group(GroupTypes.Arachnid);
			var ReptilianGroup = new Group(GroupTypes.Reptilian);
			var ElementalGroup = new Group(GroupTypes.Elemental);
			var AbyssGroup = new Group(GroupTypes.Abyss);
			var DragonLordsGroup = new Group();
			var NecroMastersGroup = new Group(GroupTypes.NecroMasters);
			var FairieGroup = new Group(GroupTypes.Fairie);
			var PlantGroup = new Group(GroupTypes.Plant);
			var UnderworldGroup = new Group(GroupTypes.Underworld);
		...
		public int Player
		{
			get => GetFactionLevel(GroupTypes.Player);
		...
		public int Humanoid
		{
			get => GetFactionLevel(GroupTypes.Humanoid);
		...
		public int Undead
		{
			get => GetFactionLevel(GroupTypes.Undead);
		...
		{
			get => GetFactionLevel(GroupTypes.Reptilian);
		...
		public int Arachnid
		{
			get => GetFactionLevel(GroupTypes.Arachnid);
		...
		{
			get => GetFactionLevel(GroupTypes.Elemental);
		...
		public int Abyss
		{
			get => GetFactionLevel(GroupTypes.Abyss);
		...
		{
			get => GetFactionLevel(GroupTypes.DragonLords);
		...
		{
			get => GetFactionLevel(GroupTypes.NecroMasters);
		...
		public int Fairie
		{
			get => GetFactionLevel(GroupTypes.Fairie);
		...
		public int Plant
		{
			get => GetFactionLevel(GroupTypes.Plant);
		...
		{
			get => GetFactionLevel(GroupTypes.Underworld);
		...
		private static int
			maxFaction = 25000; // this sets the faction level above which mobs will no longer acquire the target
		...
		public static bool CheckAcquire(Mobile mob, Mobile target)
		{
			...
			var facvalue = (int)GetScaledFaction(target, mob, -baseValue, maxFaction, 1);
			...
			var cantarget = baseValue + facvalue < Utility.Random(baseValue + maxFaction);
		...
		public static double GetScaledFaction(Mobile from, Mobile mob, double min, double max, double scale)
		{
			...
			var x = XmlAttach.FindAttachment(from, typeof(XmlMobFactions), "Standard") as XmlMobFactions;
			if (x != null)
			{
				//if(x != null)
				//{
				...
				if (dlist != null && dlist.Count > 0)
				{
					...
					var dglist = DynamicFindGroups(mob);
		...
		public static int GetCredits(Mobile m)
		{
			...
			var x = XmlAttach.FindAttachment(m, typeof(XmlMobFactions), "Standard") as XmlMobFactions;
		...
		public static int GetFactionLevel(Mobile m, GroupTypes grouptype)
		{
			...
			if (x != null)
			{
				...
				foreach (var g in x.FactionList)
		...
		private static Dictionary<Type, List<GroupTypes>> GroupTypeHash = new Dictionary<Type, List<GroupTypes>>();
		...
		public static List<GroupTypes> FindGroups(Mobile m)
		{
			...
			list = new List<GroupTypes>();

			foreach (var g in KillGroups)
		...
		public static void VerboseMobFactions_OnCommand(CommandEventArgs e)
		{
			...
			var x = XmlAttach.FindAttachment(e.Mobile, typeof(XmlMobFactions), "Standard") as XmlMobFactions;
			if (x != null)
			{
				if (e.Arguments.Length > 0)
					try
					{
						x.verboseMobFactions = Boolean.Parse(e.Arguments[0]);
					}
					catch { }
		...
		public static void CheckMobFactions_OnCommand(CommandEventArgs e)
		{
			// get the mob factions attachment
			var x = XmlAttach.FindAttachment(e.Mobile, typeof(XmlMobFactions), "Standard") as XmlMobFactions;
			if (x != null) e.Mobile.SendMessage("{0}", x.OnIdentify(e.Mobile));
		}
		...
		public static void RemoveAllMobFactions_OnCommand(CommandEventArgs e)
		{
			...
			foreach (var m in World.Mobiles.Values)
				if (m.Player)
				{
					...
					{
						foreach (var x in list) x.Delete();
						count++;
					}
		...
		private static Dictionary<Type, List<Group>> GroupHash = new Dictionary<Type, List<Group>>();
		...
		public override void OnKill(Mobile killed, Mobile killer)
		{
			...
			var value = (double)(killed.Fame / 1000.0);
			if (value <= 0) value = 1;

			// calculates credits gained in a similar way
			var cval = (int)(killed.Fame * m_CreditScale);
			if (cval <= 0) cval = 1;
			...
			foreach (var g in al)
			{
				...
				if (g.Allies != null && g.Allies.Length > 0)
					for (var i = 0; i < g.Allies.Length; i++)
					{
						...
						var p = GetFactionLevel(ally.GroupType) - facloss;
				...
				if (g.Opponents != null && g.Opponents.Length > 0)
					for (var i = 0; i < g.Opponents.Length; i++)
					{
						var opp = g.Opponents[i];
						...
						var p = GetFactionLevel(opp.GroupType) + facgain;
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				...
				case 0:
					...
					var newcount = (int)GroupTypes.End_Unused;
					;
					...
					for (var j = 0; j < newcount; j++) FactionList[j] = new GroupStatus((GroupTypes)j);
		...
		private class MobFactionGump : Gump
		{
			...
			public MobFactionGump(Mobile from, XmlMobFactions a, string text) : base(0, 0)
			{
				...
				if (a != null) AddLabel(200, 2, 30, String.Format("Available Credits: {0}", a.Credits));
		...
		public override string OnIdentify(Mobile from)
		{
			...
			if (AttachedTo is Mobile && @from != (Mobile)AttachedTo && @from != null &&
			    @from.AccessLevel == AccessLevel.Player) return null;

			// display the faction status in a gump
			var gumpmsg = new StringBuilder();
