	...
	public class TeamKotHGauntlet : BaseChallengeGame
	{
		...
		public void CheckForDisqualification()
		{
			...
			foreach (IChallengeEntry entry in Participants)
			{
				...
				if (entry.Status == ChallengeStatus.Disqualified) ClearChallenge(entry.Participant);
			}
		...
		public void CheckForKingOfTheHill()
		{
			...
			foreach (Mobile p in mlist)
				for (var i = 10; i < 20; i++)
				{
					var x = p.Location.X + i * (Utility.RandomBool() ? 1 : -1);
					var y = p.Location.Y + i * (Utility.RandomBool() ? 1 : -1);
			...
			if (elist.Count == 1)
			{
				var entry = (IChallengeEntry)elist[0];
