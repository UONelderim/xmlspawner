		...
		public override void DoTransmute(Mobile from, Recipe r)
		{
			...
			switch (rid)
			{
				case Recipes.PowerScrollSkillChange:
				{
					...
					var newps = PowerScroll.CreateRandom((int)(value - 100), (int)(value - 100));
				...
				{
					// get the target value from one of the powerscrolls
					var slist = FindItemsByType(typeof(SmallBOD), true);
				...
				{
					// get the target value from one of the powerscrolls
					var slist = FindItemsByType(typeof(LargeBOD), true);
				...
				case Recipes.UpgradeAncientAugment:
				{
					...
					if (Items.Count > 0) augment = Items[0] as BaseSocketAugmentation;
					...
					else if (augment is AncientAmethyst) DropItem(new LegendaryAmethyst());
				...
				case Recipes.UpgradeLegendaryAugment:
				{
					...
					else if (augment is LegendaryAmethyst) DropItem(new MythicAmethyst());
				...
				case Recipes.UpgradeCrystalAugment:
				{
					...
					else if (augment is KalCrystal) DropItem(new RadiantKalCrystal());
				...
				case Recipes.RecoverAugmentation:
				{
					...
					var b = FindItemByType(typeof(BaseArmor), true);
					if (b == null)
						b = FindItemByType(typeof(BaseWeapon), true);
					if (b == null)
						b = FindItemByType(typeof(BaseJewel), true);

					var a = XmlAttach.FindAttachment(b, typeof(XmlSockets)) as XmlSockets;
					if (a == null)
					{
					...
					var augment = a.RecoverRandomAugmentation(from, b);
				...
				case Recipes.SocketWeapon:
				{
					...
					var b = FindItemByType(typeof(BaseWeapon), true);
					var a = XmlAttach.FindAttachment(b, typeof(XmlSockets));
					if (a != null)
					{
						// if so then forget it
