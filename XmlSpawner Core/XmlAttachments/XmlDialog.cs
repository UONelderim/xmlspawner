...
namespace Server.Engines.XmlSpawner2
{
	public class XmlDialog : XmlAttachment
	{
		...
		private SpeechEntry GetEntry(int entryid)
		{
			...
			if (m_SpeechEntries == null) m_SpeechEntries = new ArrayList();
		...
		private bool ValidSpeechTrig(Mobile m)
		{
			...
			var allownpctrigger = false;
			if (CurrentEntry != null) allownpctrigger = CurrentEntry.AllowNPCTrigger;
		...
		public string Keywords
		{
			get
			{
				...
				if (m_CurrentEntry == null) return null;
		...
		public int SpeechHue
		{
			get
			{
				if (m_CurrentEntry == null) return 0;
		...
		public string DependsOn
		{
			...
			{
				// return the keyword string for the current entry
				if (m_CurrentEntry == null) return "-1";
		...
		public bool LockConversation
		{
			get
			{
				if (m_CurrentEntry == null) return true;
		...
		public MessageType SpeechStyle
		{
			get
			{
				if (m_CurrentEntry == null) return MessageType.Regular;
		...
		public bool AllowNPCTrigger
		{
			get
			{
				if (m_CurrentEntry == null) return false;
		...
		public bool CheckDependsOn(SpeechEntry s, int id)
		{
			...
			for (var i = 0; i < args.Length; i++)
				try
				{
					if (Int32.Parse(args[i].Trim()) == id) return true;
				}
				catch { }
		...
		private SpeechEntry FindMatchingKeyword(Mobile from, string keyword, int currententryid)
		{
			...
			if (matchlist.Count > 0)
			{
				...
				var select = Utility.Random(matchlist.Count);
		...
		private void ExecuteGump(Mobile mob, string gumpstring)
		{
			...
			var substitutedtypeName = BaseXmlSpawner.ApplySubstitution(null, this, mob, gumpstring);
			...
			if (typeName == "GUMP")
			{
				BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, true, mob, loc, map,
					new XmlGumpCallback(DialogGumpCallback), out status_str, 0);
		...
		private void ExecuteAction(Mobile mob, string action)
		{
			...
			var substitutedtypeName = BaseXmlSpawner.ApplySubstitution(null, this, mob, action);
			var typeName = BaseXmlSpawner.ParseObjectType(substitutedtypeName);
			...
			if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
				BaseXmlSpawner.SpawnTypeKeyword(AttachedTo, TheSpawn, typeName, substitutedtypeName, true, mob, loc,
					map, out status_str);
			else
			{
		...
		public override void OnSpeech(SpeechEventArgs e)
		{
			...
			if (CurrentEntry != null) lockconversation = CurrentEntry.LockConversation;
		...
		public override void OnMovement(MovementEventArgs e)
		{
			...
			if (m_Running && m_ProximityRange >= 0 && ValidMovementTrig(m) && !IsActive && !m_HoldProcessing)
			{
				...
				{
					var pause = TimeSpan.FromSeconds(0);
		...
		private void DelayedSpeech(object state)
		{
			...
			if (matchentry != null)
			{
				...
				var text = BaseXmlSpawner.ApplySubstitution(null, this, m, CurrentEntry.Text);

				if (text != null)
				{
					// dont know why emote doesnt work, but we'll just do it manually
					if (CurrentEntry.SpeechStyle == MessageType.Emote) text = String.Format("*{0}*", text);
					...
					{
						var speechhue = 0x3B2;
						if (CurrentEntry.SpeechHue >= 0) speechhue = CurrentEntry.SpeechHue;
					...
					else if (AttachedTo is Mobile)
					{
						...
						var speechhue = ((Mobile)AttachedTo).SpeechHue;
						if (CurrentEntry.SpeechHue >= 0) speechhue = CurrentEntry.SpeechHue;
				...
				if (CurrentEntry.Action != null && CurrentEntry.Action.Length > 0)
				{
					...
					for (var j = 0; j < args.Length; j++) ExecuteAction(m, args[j]);
				}
		...
		public bool ProcessSpeech(Mobile m, string speech)
		{
			...
			var matchentry = FindMatchingKeyword(m, speech, ID);
			...
			{
				var notfound = BaseXmlSpawner.CheckForNotCarried(m, NoTriggerOnCarried) || matchentry.IgnoreCarried;
			...
			var prepause = 1; // 1 sec by default
		...
		public void DoLoadNPC(Mobile from, string filename)
		{
			...
			if (File.Exists(dirname) == true)
			{
				...
				if (ds.Tables != null && ds.Tables.Count > 0)
				{
					...
					{
						var dr = ds.Tables[NPCPointName].Rows[0];

						try
						{
							if (AttachedTo is Item)
								((Item)AttachedTo).Name = (string)dr["Name"];
							else if (AttachedTo is Mobile) ((Mobile)AttachedTo).Name = (string)dr["Name"];
						}
						catch { }

						try { ProximityRange = Int32.Parse((string)dr["ProximityRange"]); }
						catch { }

						try { ResetRange = Int32.Parse((string)dr["ResetRange"]); }
						catch { }
						...
						try { m_AllowGhostTriggering = Boolean.Parse((string)dr["AllowGhost"]); }
						catch { }

						try { m_SpeechPace = Int32.Parse((string)dr["SpeechPace"]); }
						catch { }

						try { Running = Boolean.Parse((string)dr["Running"]); }
						catch { }
						...
						var entrycount = 0;
						try { entrycount = Int32.Parse((string)dr["SpeechEntries"]); }
						catch { }
					}
					...
					if (ds.Tables[NPCPointName] != null && ds.Tables[NPCPointName].Rows.Count > 0)
					{
						...
						foreach (DataRow dr in ds.Tables[SpeechPointName].Rows)
						{
							...
							try { s.EntryNumber = Int32.Parse((string)dr["EntryNumber"]); }
							catch { }

							try { s.ID = Int32.Parse((string)dr["ID"]); }
							catch { }
							...
							try { s.Pause = Int32.Parse((string)dr["Pause"]); }
							catch { }

							try { s.PrePause = Int32.Parse((string)dr["PrePause"]); }
							catch { }

							try { s.LockConversation = Boolean.Parse((string)dr["LockConversation"]); }
							catch { }

							try { s.IgnoreCarried = Boolean.Parse((string)dr["IgnoreCarried"]); }
							catch { }

							try { s.AllowNPCTrigger = Boolean.Parse((string)dr["AllowNPCTrigger"]); }
							catch { }
							...
							try { s.SpeechHue = Int32.Parse((string)dr["SpeechHue"]); }
							catch { }
		...
		public void DoSaveNPC(Mobile from, string filename, bool updateconfig)
		{
			...
			var dr = ds.Tables[NPCPointName].NewRow();

			// Populate the npc data
			if (AttachedTo is Item)
				dr["Name"] = (string)((Item)AttachedTo).Name;
			else if (AttachedTo is Mobile) dr["Name"] = (string)((Mobile)AttachedTo).Name;

			dr["Running"] = (bool)Running;
			dr["ProximityRange"] = (int)m_ProximityRange;
			dr["ResetRange"] = (int)m_ResetRange;
			dr["TriggerOnCarried"] = (string)TriggerOnCarried;
			dr["NoTriggerOnCarried"] = (string)NoTriggerOnCarried;
			dr["AllowGhost"] = (bool)m_AllowGhostTriggering;
			dr["SpeechPace"] = (int)SpeechPace;
			dr["ResetTime"] = (double)ResetTime.TotalMinutes;
			dr["ConfigFile"] = (string)ConfigFile;
			var entrycount = 0;
			if (SpeechEntries != null) entrycount = SpeechEntries.Count;
			...
			{
				var s = (SpeechEntry)SpeechEntries[i];
			...
			if (File.Exists(dirname) == true)
			{
				...
				if (from != null) @from.SendGump(new ConfirmSaveGump(this, ds, dirname, filename, updateconfig));
			}
			else
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			if (m_SpeechEntries != null)
			{
				...
				foreach (SpeechEntry s in m_SpeechEntries) writer.Write(s.IgnoreCarried);
			}
			else
				writer.Write((int)0);
			...
			{
				writer.Write((int)m_SpeechEntries.Count);
				foreach (SpeechEntry s in m_SpeechEntries) writer.Write(s.SpeechHue);
			}
			else
				writer.Write((int)0);
			...
			{
				writer.Write((int)m_SpeechEntries.Count);
				foreach (SpeechEntry s in m_SpeechEntries) writer.Write(s.Gump);
			}
			else
				writer.Write((int)0);
			...
			{
				writer.Write((int)m_SpeechEntries.Count);
				foreach (SpeechEntry s in m_SpeechEntries) writer.Write(s.Condition);
			}
			else
				writer.Write((int)0);
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				...
				{
					var count = reader.ReadInt();
					if (version < 8) m_SpeechEntries = new ArrayList();
					for (var i = 0; i < count; i++)
						if (version < 8)
						{
							var newentry = new SpeechEntry();
						...
						{
							var newentry = (SpeechEntry)m_SpeechEntries[i];
				...
				{
					var count = reader.ReadInt();
					if (version < 6) m_SpeechEntries = new ArrayList();
					for (var i = 0; i < count; i++)
						if (version < 6)
						{
							var newentry = new SpeechEntry();
				...
				{
					var count = reader.ReadInt();
					if (version < 5) m_SpeechEntries = new ArrayList();
					for (var i = 0; i < count; i++)
						if (version < 5)
						{
							var newentry = new SpeechEntry();
				...
				case 2:
				{
					...
					var count = reader.ReadInt();
					if (version < 4) m_SpeechEntries = new ArrayList();
					for (var i = 0; i < count; i++)
						if (version < 4)
						{
							var newentry = new SpeechEntry();
				...
				case 0:
				{
					...
					var count = reader.ReadInt();
					if (version < 2) m_SpeechEntries = new ArrayList();
					for (var i = 0; i < count; i++)
						if (version < 2)
						{
							var newentry = new SpeechEntry();
					...
					var isrunning = reader.ReadBool();
					if (isrunning)
					{
						var trigmob = reader.ReadMobile();
